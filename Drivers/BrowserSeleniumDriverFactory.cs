...
namespace Lesson3_Exercise.Drivers
{
	public class BrowserSeleniumDriverFactory
	{
		...
		public IWebDriver GetForBrowser(int browserId)
		{
			...
			{
				if(tuple.Key.Equals("browserName"))
				{
					Console.WriteLine(browserId + " : " + tuple.Value.ToString());
					caps.SetCapability("name", "Parallel Test - " + tuple.Value.ToString());
				}
